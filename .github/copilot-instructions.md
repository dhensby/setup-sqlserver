...
**Build output:** `@vercel/ncc` bundles everything into `lib/main/index.js`, which is what `action.yml` references. The `lib/` directory is committed to the repository.
...
- **Imports:** Use the `type` keyword for type-only imports (e.g., `import type { Foo } from './bar'`). Imports are ordered alphabetically by module path, grouped as: `node:` built-ins first, then external packages, then local file imports.
